...
//	출처 : 한국정보올림피아드
		...
		int n = sc.nextInt(); // 문제 수 입력
		int num[] = new int[n]; // 문제 수 만큼 배열의 공간 만들기
		int count = 0; // 답을 연속해서 맞을경우 값을 올려줄 count 변수
		int result = 0; // 결과가 담길 result 변수 (총점)
		for (int i = 0; i < num.length; i++) {
			num[i] = sc.nextInt(); // 반복문을 통해 배열의 크기만큼 공간에 입력해주기
		}
		for (int i = 0; i < num.length; i++) { // 배열의 크기만큼 반복
			if (num[i] == 1) { // 배열의 i번째의 값이 1일 경우
				count++; // 숫자 카운트
			} else {
				count = 0; // 1이 아니라면 문제를 틀린거기 때문에 카운트를 초기화
			}
			result = result + count; // 조건문이 끝나면 총점에 연속점수(또는 그냥 점수) 더해주기
		}
		System.out.println(result); // 반복문이 끝나면 출력
